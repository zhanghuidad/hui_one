...
static int   port = 0;
static int   bitrate = 250 * 1000;   ///波特率与TX2,250000
...
int main(int argc, char **argv)
{
    ...
    byte *psendframe = (byte *)&sendframe;   ///byte *psendframe = sendframe不行么试下
    byte *precvframe = (byte *)&recvframe;  ///byte有定义，就是u8
    u_canframe_data_t *psend_data = (u_canframe_data_t *)sendframe.data;  ///？？？？？
    const int can_frame_len = sizeof(S_CanFrame); 

    pid_t pid = -1;   ///pid_t自带类型
    int   status;      ///？
    ...
    if (parse_options(argc, argv)) ///函数 return 0: normal; return 1: 终止程序运行
    {
        usage();    return  0;
    }

    if (!find_can(port))  ///函数只知道是检查有无can设备
    {
        sprintf(buf, "\n\t错误：CAN%d设备不存在\n\n", port + 1);
        panic(buf);
        return  -1;
    ...
    send_socket_fd = socket_connect(port);  /// 绑定sock，然后监听端口返回监听 套接字 文件描述符
    ...
    set_can_filter();  ///can过滤器？？原理也没太
    set_can_loopback(send_socket_fd, lp);  //默认本地回环

    printf_head();   ///打印各种信息，can口号，bitrate
    ...
    sendframe.can_dlc = dlc;  ///数据长度
    memcpy(sendframe.data, send_frame_data, dlc);

    ///-995创建共享内存
    segment_id = shmget(IPC_PRIVATE, sizeof(int), S_IRUSR | S_IWUSR);// allocate memo///分配备忘录
