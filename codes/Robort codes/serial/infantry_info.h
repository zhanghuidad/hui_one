...
/**
  * @brief  the data of UWB Position(0x0008)
  */
typedef struct {
  float x;
  float y;
  float z;
  float yaw;
} __attribute__((packed)) UWBPosition;
...
/**
  * @brief  LaserScan information(0x00C1)
  */
#define LASER_SCAN_POINTS 36
typedef struct {
  int16_t angle[LASER_SCAN_POINTS];//deg
  uint16_t dis[LASER_SCAN_POINTS];//cm
} __attribute__((packed)) LaserScanPoints;
