...
rrts::common::ErrorInfo LocalPlannerNode::Init() {
  ...
  rrts::common::ReadProtoFromTextFile(full_path.c_str(), &local_algorithms);
  ...
  vel_pub_ = local_planner_nh_.advertise<rm_msgs::TwistAccel>("/cmd_vel_acc", 5);

  return rrts::common::ErrorInfo(rrts::common::ErrorCode::OK);
}

void LocalPlannerNode::ExcuteCB(const rm_msgs::LocalPlannerGoal::ConstPtr &command) {
  rrts::common::ErrorInfo error_info = GetErrorInfo();
  NodeState node_state = GetNodeState();

  if (node_state == NodeState::FAILURE) {
    rm_msgs::LocalPlannerFeedback feedback;
    rm_msgs::LocalPlannerResult result;
