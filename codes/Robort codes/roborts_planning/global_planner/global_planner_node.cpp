...
ErrorInfo GlobalPlannerNode::Init() {
  ...
  if (!rrts::common::ReadProtoFromTextFile(full_path.c_str(),
...
void GlobalPlannerNode::GoalCallback(const rm_msgs::GlobalPlannerGoal::ConstPtr &msg) {
  ...
  while (ros::ok()) {
    ...
    if(node_state == NodeState::RUNNING || node_state == NodeState::SUCCESS || node_state == NodeState::FAILURE) {
      rm_msgs::GlobalPlannerFeedback feedback;
      rm_msgs::GlobalPlannerResult result;
