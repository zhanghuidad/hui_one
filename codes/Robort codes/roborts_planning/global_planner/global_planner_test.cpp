...
class GlobalPlannerTest{
 public:
  ...
  void DoneCallback(const actionlib::SimpleClientGoalState& state,  const rm_msgs::GlobalPlannerResultConstPtr& result){
    ROS_INFO("The goal is done with %s!",state.toString().c_str());
  }
  void ActiveCallback() {
    ROS_INFO("Action server has recived the goal, the goal is active!");
  }
  void FeedbackCallback(const rm_msgs::GlobalPlannerFeedbackConstPtr& feedback){
 ...
 private:
  ...
  rm_msgs::GlobalPlannerGoal command_;
  actionlib::SimpleActionClient<rm_msgs::GlobalPlannerAction> global_planner_actionlib_client_;
};
